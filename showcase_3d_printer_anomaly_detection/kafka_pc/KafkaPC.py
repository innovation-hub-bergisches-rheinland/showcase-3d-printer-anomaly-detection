...
class KafkaPC:
    def __init__(self, config_path, config_section):
        ...
        self.schema_path = os.path.join(dirs.site_config_dir, "schema")
        ...
        if self.config.get("OUT_TOPIC"):
            ...
            self.producer = KafkaProducer(
                linger_ms=50, bootstrap_servers=[self.config["KAFKA_BROKER_URL"]]
            )
    ...
    def read_avro_schema(self, schema):
        schema = os.path.join(self.schema_path, schema)
    ...
    def send_msg(self, data, key=0, topic=None):
        ...
        # if no topic is provided, the first topic in the list is used as
        # default
        if topic is None:
